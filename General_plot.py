...
class GeneralPlot:
    ...
    first_range_selector = Select(title='Please select your variable for range selection', value="(select)", options=[], width=150, height=50)
    first_range_slider = RangeSlider(start=0, end=1, value=(0,1), title="")
    ...
    def upload_data(self, attr, old, new):
        ...
        self.first_range_selector.options = self.plot_data.numeric_var
    ...
    def convert_bool(self, value_list):
        res = []
        for x in value_list:
            if (x == '0') or (x == '0.0'):
                res.append(False)
    ...
    def apply_filter(self):
        selected = pd.DataFrame(self.plot_data.source_backup.data)
        ...
        for c in self.filter_widgets.children:
            ...
            if selected_filter in self.plot_data.bool_list:
                if (selected_filter == 'tasks') or (selected_filter == 'subspec'):
                    ...
                    selected_filter_values = self.convert_bool(selected_filter_values)
    ...
    def __init__(self, plot_data):
        ...
        # first_range_selector.on_change('value', functools.partial(update_slider, slider=first_range_slider))
